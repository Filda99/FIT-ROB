...
"""
    This file provides the simulator for the exploration workshop
"""

import tkinter as tk
from datetime import datetime
from tp_exploration.utils import set_target, go_2_target
...
class Simulator:
    ...
    def __init__(self, parameters):
        ...
        self.screen.title("Mobile robotics - Frontier Exploration")  # define the window name
        ...
        self.cb_lidar = tk.Checkbutton(self.screen, text="LiDAR",
        ...
        # getting the parameters and initialing the simulator
        try:
            ...
            # if some parameters are missing an error is raised
            print(ae)
            exit(0)

        self.auto_map = False  # to start automatically mapping the environment
        # the input controls for the robot
        self.cmd = [0, 0]

        # we do a first measurement set at the initial position of the robot
        self.measurements = self.sensor.get_measurements(self.robot.pose, self.environment)  # the measurements set
        self.target = None  # to move the robot autonomously
    ...
    def update_simulator(self):
        ...
        if self.auto_map:  # if we started to process the MCL (it is done by pressing the start button)
            self.map.add_measurements(self.robot.pose, self.measurements)
            self.map.remove_isolated_frontiers()
            self.map.compute_costs()
        if self.auto_move.get() == 1:
            self.target = set_target(self.robot, self.map)
            self.cmd = go_2_target(self.robot, self.target)
    ...
    def draw(self):
        ...
        if self.display_map.get() == 1:
            drawing_fct.draw_exploration_map(self.canvas, self.map)
        ...
        if self.display_lidar.get() == 1:
            drawing_fct.draw_lidar_measurements(self.canvas, self.measurements, self.robot.pose, self.environment)

        # to draw the actual robot
        if self.display_robot.get() == 1:
            drawing_fct.draw_robot(self.canvas, self.robot, self.environment)

        # to draw the target (where the robot is supposed to go)
        if self.target is not None:
            drawing_fct.draw_point(self.canvas, self.target, 0.05, "green", self.environment)

    def btn_test_event(self, _):
        """ function called when pressing the test button, it can be used for tests
        PARAMETERS:
            _: the button event, not used
        """
        # You can put some test code here!
        print("Test button has been pressed", self.auto_map)

    def btn_start_event(self, _):
        """ function called when pressing the start button, it changes the value of auto map flag
        PARAMETERS:
            _: the button event, not used
        """
        self.auto_map = True
