...
def draw_robot(canvas, robot, world_dimension):
    ...
    z_left_top = z_real_2_draw(canvas, robot.pose.y - robot.robot_width, world_dimension)

    x_right_bot = x_real_2_draw(canvas, robot.pose.x + robot.robot_width, world_dimension)
    z_right_bot = z_real_2_draw(canvas, robot.pose.y + robot.robot_width, world_dimension)
    ...
    z_start = z_real_2_draw(canvas, robot.pose.y, world_dimension)
    x_end = x_real_2_draw(canvas, robot.pose.x + cos(robot.pose.theta) * robot.robot_width, world_dimension)
    z_end = z_real_2_draw(canvas, robot.pose.y + sin(robot.pose.theta) * robot.robot_width, world_dimension)
...
def draw_particles(canvas, particles, world_dimension):
    ...
    for particle in particles:
        draw_point(canvas, particle, size=0.05, color="grey", world_dimension=world_dimension)


def draw_landmarks(canvas, world_dimension):
    """ Function to draw the global landmarks as points. """
    for landmark in global_landmarks:
        draw_point(canvas, landmark, size=0.05, color="red", world_dimension=world_dimension)